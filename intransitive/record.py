...
FORMAT = 'Intransitive-PGN-1'
RULES = 'threefold;30-plies-without-capture'
...
def export_record(moves, board, config, *, opponent='local', human_player=0, last_ai=None):
    ...
    tags = dict(Variant='Intransitive', Format=FORMAT, Rules=RULES,
...
def load_record(text):
    ...
    if (tags.get('Variant'), tags.get('Format'), tags.get('Rules')) != ('Intransitive', FORMAT, RULES):
        raise ValueError('Expected a copied Intransitive PGN-1 record with supported draw rules')
    ...
    if tags.get('PlyCount') != str(len(actions)) or result_token(board) != tokens[-1]:
        raise ValueError('Move history does not match the recorded ply count or result')
    if tags.get('StateSHA256') != state_hash(states[-1]):
    ...
    if last_ai is not None:
        ...
        if (type(ply) is not int or not 0 <= ply < len(actions)
                ...
                or last_ai.get('state_sha256') != state_hash(states[ply])):
