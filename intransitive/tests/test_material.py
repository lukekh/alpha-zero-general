...
class MaterialTests(unittest.TestCase):
    ...
    def variants(self):
        ...
        `lmr_enabled`, the two pruning switches require
        `selective_evaluator_enabled` because this configuration's evolved
        scales are outside their calibrated margins, and the two version strings
        have no neighbouring accepted value at all. The two enums move within
        their own range; the six constrained fields move their sibling with
        them. Every other field moves alone.
        """
        special = {
